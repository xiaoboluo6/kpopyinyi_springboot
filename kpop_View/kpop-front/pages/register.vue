...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitRegister(){
        // 先应该判断填写信息有没有缺失
        if(this.params.mobile == '' || this.params.code == '' || this.params.nickname == '' || this.params.password == ''){
          //提示信息填写不全
          this.$message({
            type: 'warning',
            message: "信息填写不全"
          ...
          .then(response =>{
            // 这里返回的都是 R  一个是R.ok()  一个是R.error  我们可以读取response里面的内容进行判断
          ...
          .catch(error => {   // 这里应该是前端的报错 后端报错 交给上面的逻辑进行判断
            // 捕获并处理错误
            this.$message({
              type: 'error',
              message: '注册失败，请稍后重试'
...
/*         registerApi.sendCode(this.params.mobile)
        ...
        // 这里选择不请求  默认验证码是1379 输入1379就是正确的 这样可以不启动edumsm服务
        this.sending = false
        this.$message({
            type: 'success',
            message: "新用户注册请联系管理员"
      ...
      timeDown() {
        let result = setInterval(() => {
          --this.second;
          this.codeTest = this.second + "s后可再次发送"
          if (this.second < 1) {
